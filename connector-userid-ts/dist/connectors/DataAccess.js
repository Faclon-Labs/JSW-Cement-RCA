...
import { GET_DEVICE_DETAILS_URL, GET_DEVICE_METADATA_URL, GET_USER_INFO_URL, GET_DP_URL, GET_FIRST_DP, INFLUXDB_URL, GET_LOAD_ENTITIES, MAX_RETRIES, RETRY_DELAY, Protocol } from '../utils/constants.js';
export default class DataAccess {
     ...
     */
    constructor({ userId, dataUrl, dsUrl, onPrem = false, tz = "UTC" }) {
        this.userId = userId;
    ...
    timeToUnix(time = null, timezone = "UTC") {
        ...
        if (typeof time === "number") {
            ...
            if (time <= 0 || String(time).length <= 10) {
                throw new Error("Unix timestamp must be a positive integer in milliseconds, not seconds.");
            }
            return time;
        }
    ...
    formatSensorData(data) {
        ...
        for (const sensorData of Object.values(data || {})) {
            if (Array.isArray(sensorData)) {
                sensorData.forEach(({ time, sensor, value }) => result.push({ time: time || '', sensor: sensor || '', value: value || null }));
            }
     ...
     */
    getCleanedTable(options) {
        const { data, alias = false, cal = false, deviceId, sensorList, onPrem = false, unix = false, metadata = null, pivotTable = true, } = options;
        ...
        if (sensorList && sensorList.length > 0) {
            ...
            cleanedData = cleanedData.filter((row) => sensorList.includes(row.sensor) || sensorList.includes(row.sensorId));
        }
        ...
        if (pivotTable) {
            ...
            timestamps.forEach((timestamp) => {
                const rowsAtTimestamp = cleanedData.filter((row) => (row.timestamp || row.time) === timestamp);
    ...
    async getUserInfo(onPremOverride = null) {
        ...
        const url = GET_USER_INFO_URL.replace("{protocol}", protocol).replace("{data_url}", this.dataUrl);
        try {
    ...
    async getDeviceDetails(onPremOverride = null) {
        ...
        const protocol = onPrem ? Protocol.HTTP : Protocol.HTTPS;
        // Construct the URL for the API request
        const url = GET_DEVICE_DETAILS_URL.replace("{protocol}", protocol).replace("{data_url}", this.dataUrl);
        try {
            const response = await axios.get(url, {
                headers: {
                    userID: this.userId,
...
[RESPONSE] ${JSON.stringify(body)}
     ...
     */
    async getFirstDp(options) {
        const { deviceId, sensorList = null, cal = true, startTime = null, n = 1, alias = false, unix = false, onPrem = null, } = options;
        try {
            if (n < 1)
                throw new Error("Parameter 'n' must be ≥ 1");
     ...
     */
    async getDp(options) {
        const { deviceId, sensorList = null, n = 1, cal = true, endTime = null, alias = false, unix = false, onPrem = null, } = options;
        try {
            if (n < 1)
                throw new Error("Parameter 'n' must be ≥ 1");
            const useOnPrem = onPrem ?? this.onPrem;
     ...
     * @throws Error if no sensor data is available for the device
     */
    async dataQuery(options) {
        const { deviceId, sensorList = null, startTime = null, endTime = null, cal = true, alias = false, unix = false, onPrem = null, } = options;
        try {
            const useOnPrem = onPrem ?? this.onPrem;
     ...
     * @returns Array of sensor data points
     */
    async _influxdb(options) {
        const { deviceId, startTime, endTime, alias = false, cal = true, unix = false, sensorList = [], metadata = null, onPrem = null, } = options;
        ...
        try {
            ...
            while (cursor?.start && cursor?.end) {
                try {
                    ...
                    if (success)
                        throw new Error("Influx error: " + JSON.stringify(response.data));
        ...
        try {
            ...
            while (hasMore) {
                try {
                    ...
                    console.error(`[${error.name}] Retry Count: ${retry}, ${error.message} ${this.errorMessage(errorResponse, baseUrl)}`);
                    if (retry < MAX_RETRIES) {
                        const sleepTime = retry > 5 ? RETRY_DELAY[1] : RETRY_DELAY[0];
                        await this._sleep(sleepTime);
                    }
                    else {
                        throw new Error(`Max retries for data fetching from api-layer exceeded. ${this.errorMessage(errorResponse, baseUrl)}`);
                    }
                }
            }
